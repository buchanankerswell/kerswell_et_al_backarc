...
shp.hf.filtered <- shp.hf %>% 
  rename('Conductivity' = Conductivi, 
         ...
         'Min Depth' = minD, 
         'Max Depth' = maxD, 
         'No. of Cond.' = No__Cond_, 
         'No. of Temps' = No__Temps) %>%
  filter(Gradient < 500 & Gradient > 0 & Conductivity < 8 & `No. of Temps` < 250)
...
# Find point positions and calculate distances relative to trench
shp.hf.pos <- purrr::map_df(unique(shp.hf$segment),
                            ~shp.hf %>% filter(segment == .x) %>%
                              mutate(pos = pts_position(shp.hf %>% filter(segment == .x),
                                                        shp.sa.segs.robin.pacific %>% filter(segment == .x),
                                                        5e6,
                                                        5e6,
                                                        'updown',
                                                        'up'),
                                     t.dist = trench_distance(shp.hf %>% filter(segment == .x),
                                                              pos,
                                                              shp.sa.segs.robin.pacific %>% filter(segment == .x))))

# Find point positions and calculate distances relative to trench 
shp.hf.pos.flat <- purrr::map_df(unique(shp.hf$segment),
                                 ~shp.hf.flat %>% filter(segment == .x) %>%
                                   mutate(pos = pts_position(shp.hf.flat %>% filter(segment == .x),
                                                             shp.sa.segs.robin.pacific %>% filter(segment == .x),
                                                             5e6,
                                                             5e6,
                                                             'updown',
                                                             'up'),
                                          t.dist = trench_distance(shp.hf.flat %>% filter(segment == .x),
                                                                   pos,
                                                                   shp.sa.segs.robin.pacific %>% filter(segment == .x))))
# Plot segments
unique(shp.hf$segment) %>%
  purrr::map(~ggplot(shp.hf.pos.flat %>% filter(segment == .x)) +
               geom_sf(aes(color = Heat_Flow, shape = pos)) +
               geom_sf(data = shp.sa.segs.robin.pacific.buffer %>% filter(segment == .x), fill = NA) +
               geom_sf(data = shp.sa.segs.robin.pacific %>% filter(segment == .x)) + labs(title = .x) +
               scale_color_viridis_c(option = 'A') + theme_map())

# Plot points vs distance
unique(shp.hf$segment) %>%
  purrr::map(~ggplot(shp.hf.pos.flat %>% filter(segment == .x)) +
               geom_point(aes(x = t.dist/1000, y = Heat_Flow)) +
               labs(x = bquote(Distance~~km), y = bquote(Heat~Flow~~mWm^-2), title = .x) +
               theme_classic(base_size = 14))

# Krige individual segments______________________________________________________________
...
# Filter hf data
shp.hf.flat.filtered.aa <- shp.hf.flat %>%
  filter(segment == 'Alaska Aleutians') %>%
  filter(Gradient < 500 & Gradient > 0 & Conductivi < 8 & No__Temps < 250)

shp.hf.flat.filtered.pos.aa <- shp.hf.flat.filtered.aa %>%
  mutate(pos = pts_position(shp.hf.flat.filtered.aa,
                            shp.sa.segs.robin.pacific %>% filter(segment == 'Alaska Aleutians'),
                            5e6,
                            5e6,
                            'updown',
                            'up'),
         t.dist = trench_distance(shp.hf.flat.filtered.aa,
                                  pos,
                                  shp.sa.segs.robin.pacific %>% filter(segment == 'Alaska Aleutians')))
# Plot heat flow (map view)
shp.hf.flat.filtered.pos.aa %>% ggplot() +
  geom_sf(aes(color = Heat_Flow, shape = pos)) +
  geom_sf(data = shp.sa.segs.robin.pacific.buffer %>% filter(segment == 'Alaska Aleutians'), fill = NA) +
  geom_sf(data = shp.sa.segs.robin.pacific %>% filter(segment == 'Alaska Aleutians')) +
  labs(title = ) + scale_color_viridis_c(option = 'A') +
  theme_map()

# Plot all heat flow vs. distance to trench
shp.hf.flat.filtered.pos.aa %>% filter(segment == 'Alaska Aleutians') %>%
  ggplot() +
  geom_point(aes(x = t.dist/1000, y = Heat_Flow)) +
  labs(x = bquote(Distance~~km), y = bquote(Heat~Flow~~mWm^-2), title = 'Alaska Aleutians') +
  theme_classic(base_size = 14)

# Krige entire segment
k.aa <- krg(
  data = shp.hf.flat.filtered.pos.aa,
  lags = 100,
  lag.cutoff = 3,
  param = 'Heat_Flow',
...
k.aa$k.plot / (k.aa$v.plot + k.aa$hist) + plot_layout(heights = c(3, 1)) +
  ...
  theme(plot.title = element_text(size = 18,hjust = 0.5, margin = margin(14, 0, 0, 0)),
...
# Split segment
shp.aa.splt.seg <- shp.sa.segs.robin.pacific %>%
  filter(segment == 'Alaska Aleutians') %>%
  splt(cut.length = as.vector(l.aa)/4, buffer = F,
       buffer.dist = 500000,
       cap.style = 'ROUND') %>%
  mutate(segment = 'Alaska Aleutians', .before = geometry)

# Split buffer
shp.aa.splt.buffer <- shp.sa.segs.robin.pacific %>%
  filter(segment == 'Alaska Aleutians') %>%
  splt(cut.length = as.vector(l.aa)/4,
       buffer = T,
       buffer.dist = 500000,
       cap.style = 'ROUND') %>%
  mutate(segment = 'Alaska Aleutians', .before = geometry)
...
# Split hf data
shp.hf.aa.splt <- shp.hf.flat.filtered.aa %>% st_join(shp.aa.splt.buffer, left = F)
...
k.aa.subseg <- purrr::map(unique(shp.hf.aa.splt$subseg) %>% sort(), ~{
  krg(
    data = shp.hf.aa.splt %>% filter(subseg == .x),
    lags = 100,
    lag.cutoff = 3,
    param = 'Heat_Flow',
...
k.aa$k.plot /
  ((k.aa.subseg[[1]]$k.plot + k.aa.subseg[[2]]$k.plot) /
     (k.aa.subseg[[3]]$k.plot + k.aa.subseg[[4]]$k.plot)) +
  plot_annotation(title = 'Alaska Aleutians') &
  theme(plot.title = element_text(size = 18, hjust = 0.5, margin = margin(14, 0, 0, 0)),
        plot.background = element_rect(fill = 'transparent', color = NA),
        panel.background = element_rect(fill = 'transparent', color = NA))
...
k.aa$v.plot / ((k.aa.subseg[[1]]$v.plot + k.aa.subseg[[2]]$v.plot + theme(axis.title.y = element_blank())) /
                 (k.aa.subseg[[3]]$v.plot + k.aa.subseg[[4]]$v.plot + theme(axis.title.y = element_blank()))) +
  plot_annotation(title = 'Alaska Aleutians') &
  theme(plot.title = element_text(size = 18, hjust = 0.5, margin = margin(14, 0, 0, 0)),
        plot.background = element_rect(fill = 'transparent', color = NA),
        panel.background = element_rect(fill = 'transparent', color = NA),
...
# Filter hf data
shp.hf.flat.filtered.an <- shp.hf.flat %>%
  filter(segment == 'Andes') %>%
  filter(Gradient < 500 & Gradient > 0 & Conductivi < 8 & No__Temps < 250)

shp.hf.flat.filtered.pos.an <- shp.hf.flat.filtered.an %>%
  mutate(pos = pts_position(shp.hf.flat.filtered.an,
                            shp.sa.segs.robin.pacific %>% filter(segment == 'Andes'),
                            5e6,
                            5e6,
                            'leftright',
                            'right'),
         t.dist = trench_distance(shp.hf.flat.filtered.an,
                                  pos,
                                  shp.sa.segs.robin.pacific %>% filter(segment == 'Andes')))
# Plot heat flow (map view)
shp.hf.flat.filtered.pos.an %>% ggplot() +
  geom_sf(aes(color = Heat_Flow, shape = pos)) +
  geom_sf(data = shp.sa.segs.robin.pacific.buffer %>% filter(segment == 'Andes'), fill = NA) +
  geom_sf(data = shp.sa.segs.robin.pacific %>% filter(segment == 'Andes')) +
  labs(title = ) + scale_color_viridis_c(option = 'A') +
  theme_map()

# Plot all heat flow vs. distance to trench
shp.hf.flat.filtered.pos.an %>% filter(segment == 'Andes') %>%
  ggplot() +
  geom_point(aes(x = t.dist/1000, y = Heat_Flow)) +
  labs(x = bquote(Distance~~km), y = bquote(Heat~Flow~~mWm^-2), title = 'Andes') +
  theme_classic(base_size = 14)

# Krige entire segment
k.an <- krg(
  data = shp.hf.flat.filtered.pos.an,
  lags = 100,
  lag.cutoff = 3,
  param = 'Heat_Flow',
...
# Save plot
ggsave('figs/an.png', device = 'png', dpi = 330, width = 11, height = 7, bg = 'transparent')
...
# Split segment
shp.an.splt.seg <- shp.sa.segs.robin.pacific %>%
  filter(segment == 'Andes') %>%
  splt(cut.length = as.vector(l.an)/4, buffer = F,
       buffer.dist = 500000,
       cap.style = 'ROUND') %>%
  mutate(segment = 'Andes', .before = geometry)

# Split buffer
shp.an.splt.buffer <- shp.sa.segs.robin.pacific %>%
  filter(segment == 'Andes') %>%
  splt(cut.length = as.vector(l.an)/4,
       buffer = T,
       buffer.dist = 500000,
       cap.style = 'ROUND') %>%
  mutate(segment = 'Andes', .before = geometry)
...
# Split hf data
shp.hf.an.splt <- shp.hf.flat.filtered.an %>% st_join(shp.an.splt.buffer, left = F)
...
k.an.subseg <- purrr::map(unique(shp.hf.an.splt$subseg) %>% sort(), ~{
  krg(
    data = shp.hf.an.splt %>% filter(subseg == .x),
    lags = 100,
    lag.cutoff = 3,
    param = 'Heat_Flow',
...
# Summarise experimental variograms
purrr::map_df(k.an.subseg,
              ~.x$variogram %>% summarise(np.max = max(np),
                                          np.min = min(np),
                                          np.mean = round(mean(np)),
                                          np.median = median(np)),
              .id = 'subseg')
...
# Composite summary plot (krige results)
k.an$k.plot +
  ((k.an.subseg[[1]]$k.plot + k.an.subseg[[2]]$k.plot) /
     (k.an.subseg[[3]]$k.plot + k.an.subseg[[4]]$k.plot)) +
  plot_annotation(title = 'Andes') &
  theme(plot.title = element_text(size = 18, hjust = 0.5, margin = margin(14, 0, 0, 0)),
        plot.background = element_rect(fill = 'transparent', color = NA),
        panel.background = element_rect(fill = 'transparent', color = NA))
...
# Save plot
ggsave('figs/an-subseg-krig.png', device = 'png', dpi = 330, width = 11, height = 10, bg = 'transparent')

# Composite summary plot (variograms)
k.an$v.plot / ((k.an.subseg[[1]]$v.plot + k.an.subseg[[2]]$v.plot + theme(axis.title.y = element_blank())) / (k.an.subseg[[3]]$v.plot + k.an.subseg[[4]]$v.plot + theme(axis.title.y = element_blank()))) +
  plot_annotation(title = 'Andes') &
...
# Save plot
ggsave('figs/an-subseg-variogram.png', device = 'png', dpi = 330, width = 11, height = 10, bg = 'transparent')