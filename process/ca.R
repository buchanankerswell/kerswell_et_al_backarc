...
# Box
shp.box.ca <-
  bbox_widen(st_bbox(shp.buf.ca),
             crs = st_crs(shp.buf.ca),
             borders = c(
               'left' = 0.1,
               'right' = 0.1,
               'top' = 0.1,
...
# # Rotated projection
# proj.rot <- rotate_proj(shp.hf.ca, rotate.cagle)
...
k <-
  model_variogram(
  data = shp.hf.ca %>% rename(hf = `heat-flow (mW/m2)`),
  lags = lags,
  lag.cutoff = lag.cutoff,
  param = 'hf',
  grid = shp.grid %>% st_crop(shp.box.ca),
  grid.rotate = rotate.cagle,
  v.mod = v.mod,
  krige = T
)
...
# Variogram
cat('\nPlotting variogram ... ')
ggsave('../figs/ca/ca_variogram.png',
...
shp.hf.pred.ca %>%
  ...
  geom_sf(data = shp.seg.ca, color = 'white', alpha = 0.6) +
  geom_sf(data = shp.con.ca, size = 0.2, color = 'white', alpha = 0.3) +
...
# Composition
p <- p1 / p2 +
  plot_annotation(title = 'Central America', tag_levels = 'a')
ggsave('../figs/ca/ca_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 7,
...
p4 <-
  ...
  scale_x_continuous(limits = c(median(shp.hf.pred.ca$HF_diff)-(3*IQR(shp.hf.pred.ca$HF_diff)), median(shp.hf.pred.ca$HF_diff)+(3*IQR(shp.hf.pred.ca$HF_diff)))) +
  annotate('text',
           x = (max(shp.hf.pred.ca$HF_diff) - min(shp.hf.pred.ca$HF_diff))/4,
           y = -Inf,
...
p <- p1 + p2 + p3 + p4 +
  ...
  plot_annotation(title = 'Central America', tag_levels = 'a')
ggsave('../figs/ca/ca_diff_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 11,
...
# Save results
cat('Saving results to process/ca.RData')
save(k, shp.hf.pred.ca, file = 'ca.RData')
