...
k <-
  model_variogram(
  data = shp.hf %>% rename(hf = `heat-flow (mW/m2)`),
  lags = lags,
...
# Variogram
cat('\nPlotting variogram ... ')
ggsave('../figs/glb/glb_variogram.png',
...
# Composition
p <- p1 / p2 +
  plot_annotation(title = 'Global', tag_levels = 'a')
ggsave('../figs/glb/glb_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 7,
...
p4 <-
  ...
  scale_x_continuous(limits = c(median(shp.hf.pred$HF_diff)-(3*IQR(shp.hf.pred$HF_diff)), median(shp.hf.pred$HF_diff)+(3*IQR(shp.hf.pred$HF_diff)))) +
  annotate('text',
           x = (max(shp.hf.pred$HF_diff) - min(shp.hf.pred$HF_diff))/4,
           y = -Inf,
...
p <- p1 + p2 + p3 + p4 +
  ...
  plot_annotation(title = 'Global', tag_levels = 'a')
ggsave('../figs/glb/glb_diff_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 11,
...
# Save results
cat('Saving results to process/glb.RData')
save(k, shp.hf.pred, file = 'glb.RData')
