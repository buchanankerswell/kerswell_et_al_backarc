...
# Box
shp.box.sp <-
  bbox_widen(st_bbox(shp.buf.sp),
             crs = st_crs(shp.buf.sp),
             borders = c(
               'left' = 0.1,
               'right' = 0.1,
               'top' = 0.1,
...
# # Rotated projection
# proj.rot <- rotate_proj(shp.hf.sp, rotate.spgle)
...
k <-
  model_variogram(
  data = shp.hf.sp %>% rename(hf = `heat-flow (mW/m2)`),
  lags = lags,
  lag.cutoff = lag.cutoff,
  param = 'hf',
  grid = shp.grid %>% st_crop(shp.box.sp),
  grid.rotate = rotate.spgle,
  v.mod = v.mod,
  krige = T
)
...
# Variogram
cat('\nPlotting variogram ... ')
ggsave('../figs/sp/sp_variogram.png',
...
shp.hf.pred.sp %>%
  ...
  geom_sf(data = shp.seg.sp, color = 'white', alpha = 0.6) +
  geom_sf(data = shp.con.sp, size = 0.2, color = 'white', alpha = 0.3) +
...
# Composition
p <- p1 + p2 +
  plot_annotation(title = 'S. Philippines', tag_levels = 'a')
ggsave('../figs/sp/sp_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 8,
...
p4 <-
  ...
  scale_x_continuous(limits = c(median(shp.hf.pred.sp$HF_diff)-(3*IQR(shp.hf.pred.sp$HF_diff)), median(shp.hf.pred.sp$HF_diff)+(3*IQR(shp.hf.pred.sp$HF_diff)))) +
  annotate('text',
           x = (max(shp.hf.pred.sp$HF_diff) - min(shp.hf.pred.sp$HF_diff))/4,
           y = -Inf,
...
p <- p1 + p2 + p3 + p4 +
  ...
  plot_annotation(title = 'S. Philippines', tag_levels = 'a')
ggsave('../figs/sp/sp_diff_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 8,
...
# Save results
cat('Saving results to process/sp.RData')
save(k, shp.hf.pred.sp, file = 'sp.RData')
