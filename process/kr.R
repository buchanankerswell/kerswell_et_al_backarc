...
# Box
shp.box.kr <-
  bbox_widen(st_bbox(shp.buf.kr),
             crs = st_crs(shp.buf.kr),
             borders = c(
               'left' = 0.1,
               'right' = 0.1,
               'top' = 0.1,
...
# # Rotated projection
# proj.rot <- rotate_proj(shp.hf.kr, rotate.krgle)
...
k <-
  krige_interp(
  data = shp.hf.kr %>% rename(hf = `heat-flow (mW/m2)`),
  lags = lags,
  lag.cutoff = lag.cutoff,
  param = 'hf',
  grid = shp.grid %>% st_crop(shp.box.kr),
  grid.rotate = rotate.krgle,
  v.mod = v.mod,
  krige = T
)
...
# Variogram
cat('\nPlotting variogram ... ')
ggsave('../figs/kr/kr_variogram.png',
...
shp.hf.pred.kr %>%
  ...
  geom_sf(data = shp.seg.kr, color = 'white', alpha = 0.6) +
  geom_sf(data = shp.con.kr, size = 0.2, color = 'white', alpha = 0.3) +
...
# Composition
p <- p1 / p2 +
  plot_annotation(title = 'Kyushu Ryukyu', tag_levels = 'a')
ggsave('../figs/kr/kr_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 7,
...
p4 <-
  ...
  scale_x_continuous(limits = c(median(shp.hf.pred.kr$HF_diff)-(3*IQR(shp.hf.pred.kr$HF_diff)), median(shp.hf.pred.kr$HF_diff)+(3*IQR(shp.hf.pred.kr$HF_diff)))) +
  annotate('text',
           x = (max(shp.hf.pred.kr$HF_diff) - min(shp.hf.pred.kr$HF_diff))/4,
           y = -Inf,
...
p <- p1 + p2 + p3 + p4 +
  ...
  plot_annotation(title = 'Kyushu Ryukyu', tag_levels = 'a')
ggsave('../figs/kr/kr_diff_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 11,
...
# Save results
cat('Saving results to process/kr.RData')
save(k, shp.hf.pred.kr, file = 'kr.RData')
