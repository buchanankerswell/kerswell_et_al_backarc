...
# Box
shp.box.km <-
  bbox_widen(st_bbox(shp.buf.km),
             crs = st_crs(shp.buf.km),
             borders = c(
               'left' = 0.1,
               'right' = 0.1,
               'top' = 0.1,
...
# # Rotated projection
# proj.rot <- rotate_proj(shp.hf.km, rotate.kmgle)
...
k <-
  krige_interp(
  data = shp.hf.km %>% rename(hf = `heat-flow (mW/m2)`),
  lags = lags,
  lag.cutoff = lag.cutoff,
  param = 'hf',
  grid = shp.grid %>% st_crop(shp.box.km),
  grid.rotate = rotate.kmgle,
  v.mod = v.mod,
  krige = T
)
...
# Variogram
cat('\nPlotting variogram ... ')
ggsave('../figs/km/km_variogram.png',
...
shp.hf.pred.km %>%
  ...
  geom_sf(data = shp.seg.km, color = 'white', alpha = 0.6) +
  geom_sf(data = shp.con.km, size = 0.2, color = 'white', alpha = 0.3) +
...
# Composition
p <- p1 + p2 +
  plot_annotation(title = 'Kamchatka Marianas', tag_levels = 'a')
ggsave('../figs/km/km_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 8,
...
p4 <-
  ...
  scale_x_continuous(limits = c(median(shp.hf.pred.km$HF_diff)-(3*IQR(shp.hf.pred.km$HF_diff)), median(shp.hf.pred.km$HF_diff)+(3*IQR(shp.hf.pred.km$HF_diff)))) +
  annotate('text',
           x = (max(shp.hf.pred.km$HF_diff) - min(shp.hf.pred.km$HF_diff))/4,
           y = -Inf,
...
p <- p1 + p2 + p3 + p4 +
  ...
  plot_annotation(title = 'Kamchatka Marianas', tag_levels = 'a')
ggsave('../figs/km/km_diff_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 11,
...
# Save results
cat('Saving results to process/km.RData')
save(k, shp.hf.pred.km, file = 'km.RData')
