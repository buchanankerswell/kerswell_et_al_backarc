...
# Box
shp.box.tn <-
  bbox_widen(st_bbox(shp.buf.tn),
             crs = st_crs(shp.buf.tn),
             borders = c(
               'left' = 0.1,
               'right' = 0.1,
               'top' = 0.1,
...
# # Rotated projection
# proj.rot <- rotate_proj(shp.hf.tn, rotate.tngle)
...
k <-
  model_variogram(
  data = shp.hf.tn %>% rename(hf = `heat-flow (mW/m2)`),
  lags = lags,
  lag.cutoff = lag.cutoff,
  param = 'hf',
  grid = shp.grid %>% st_crop(shp.box.tn),
  grid.rotate = rotate.tngle,
  v.mod = v.mod,
  krige = T
)
...
# Variogram
cat('\nPlotting variogram ... ')
ggsave('../figs/tn/tn_variogram.png',
...
shp.hf.pred.tn %>%
  ...
  geom_sf(data = shp.seg.tn, color = 'white', alpha = 0.6) +
  geom_sf(data = shp.con.tn, size = 0.2, color = 'white', alpha = 0.3) +
...
# Composition
p <- p1 + p2 +
  plot_annotation(title = 'Tonga New Zealand', tag_levels = 'a')
ggsave('../figs/tn/tn_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 7,
...
p4 <-
  ...
  scale_x_continuous(limits = c(median(shp.hf.pred.tn$HF_diff)-(3*IQR(shp.hf.pred.tn$HF_diff)), median(shp.hf.pred.tn$HF_diff)+(3*IQR(shp.hf.pred.tn$HF_diff)))) +
  annotate('text',
           x = (max(shp.hf.pred.tn$HF_diff) - min(shp.hf.pred.tn$HF_diff))/4,
           y = -Inf,
...
p <- p1 + p2 + p3 + p4 +
  ...
  plot_annotation(title = 'Tonga New Zealand', tag_levels = 'a')
ggsave('../figs/tn/tn_diff_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 8,
...
# Save results
cat('Saving results to process/tn.RData')
save(k, shp.hf.pred.tn, file = 'tn.RData')
