...
# # Rotated projection
# proj.rot <- rotate_proj(shp.hf.aa, rotate.aagle)
...
k <-
  krige_interp(
  data = shp.hf.aa %>% rename(hf = `heat-flow (mW/m2)`),
  lags = lags,
  lag.cutoff = lag.cutoff,
  param = 'hf',
  grid = shp.grid %>% st_crop(shp.box.aa),
  grid.rotate = rotate.aagle,
  v.mod = v.mod,
  krige = F
)
...
# Variogram
cat('\nPlotting variogram ... ')
ggsave('../figs/aa/aa_variogram.png',
...
shp.hf.pred.aa %>%
  ...
  geom_sf(data = shp.seg.aa, color = 'white', alpha = 0.6) +
  geom_sf(data = shp.con.aa, size = 0.2, color = 'white', alpha = 0.3) +
...
# Composition
p <- p1 / p2 +
  plot_annotation(title = 'Alaska Aleutians', tag_levels = 'a')
ggsave('../figs/aa/aa_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 7,
...
p4 <-
  ...
  scale_x_continuous(limits = c(median(shp.hf.pred.aa$HF_diff)-(3*IQR(shp.hf.pred.aa$HF_diff)), median(shp.hf.pred.aa$HF_diff)+(3*IQR(shp.hf.pred.aa$HF_diff)))) +
  annotate('text',
           x = (max(shp.hf.pred.aa$HF_diff) - min(shp.hf.pred.aa$HF_diff))/4,
           y = -Inf,
...
p <- p1 + p2 + p3 + p4 +
  ...
  plot_annotation(title = 'Alaska Aleutians', tag_levels = 'a')
ggsave('../figs/aa/aa_diff_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 11,
...
# Save results
cat('Saving results to process/aa.RData')
save(k, shp.hf.pred.aa, file = 'aa.RData')
