...
# Box
shp.box.np <-
  bbox_widen(st_bbox(shp.buf.np),
             crs = st_crs(shp.buf.np),
             borders = c(
               'left' = 0.1,
               'right' = 0.1,
               'top' = 0.1,
...
# # Rotated projection
# proj.rot <- rotate_proj(shp.hf.np, rotate.npgle)
...
k <-
  model_variogram(
  data = shp.hf.np %>% rename(hf = `heat-flow (mW/m2)`),
  lags = lags,
  lag.cutoff = lag.cutoff,
  param = 'hf',
  grid = shp.grid %>% st_crop(shp.box.np),
  grid.rotate = rotate.npgle,
  v.mod = v.mod,
  krige = T
)
...
# Variogram
cat('\nPlotting variogram ... ')
ggsave('../figs/np/np_variogram.png',
      plot = k$krige.results$variogram.plot,
      device = 'png',
      type = 'cairo',
      width = 7,
      height = 4)
...
shp.hf.pred.np %>%
  ...
  geom_sf(data = shp.seg.np, color = 'white', alpha = 0.6) +
  geom_sf(data = shp.con.np, size = 0.2, color = 'white', alpha = 0.3) +
...
# Composition
p <- p1 + p2 +
  plot_annotation(title = 'N. Philippines', tag_levels = 'a')
ggsave('../figs/np/np_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 8,
...
p4 <-
  ...
  scale_x_continuous(limits = c(median(shp.hf.pred.np$HF_diff)-(3*IQR(shp.hf.pred.np$HF_diff)), median(shp.hf.pred.np$HF_diff)+(3*IQR(shp.hf.pred.np$HF_diff)))) +
  annotate('text',
           x = (max(shp.hf.pred.np$HF_diff) - min(shp.hf.pred.np$HF_diff))/4,
           y = -Inf,
...
p <- p1 + p2 + p3 + p4 +
  ...
  plot_annotation(title = 'N. Philippines', tag_levels = 'a')
ggsave('../figs/np/np_diff_comp.png',
       plot = p,
       device = 'png',
       type = 'cairo',
       width = 8,
...
# Save results
cat('Saving results to process/np.RData')
save(k, shp.hf.pred.np, file = 'np.RData')
